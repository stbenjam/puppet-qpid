...
describe 'qpid::client::config' do
  context 'on redhat' do
    ...
    context 'on hub with ssl' do
      let :pre_condition do
        'class {"qpid::router":
           ...
           connector_addr   => "1.2.3.4",
           ssl              => true,
           ssl_ca_cert      => "/etc/pki/ca/ca.pem",
           ssl_cert         => "/etc/pki/cert.pem",
           ssl_key          => "/etc/pki/private/key.key",
         }'
      end
      ...
      it 'should configure on-demand listener and broker connector' do
        ...
        content.split("\n").reject { |c| c =~ /(^#|^$)/ }.should == [
          ...
          'ssl-profile {',
          '    name: router-ssl',
          '    cert-db: /etc/pki/ca/ca.pem',
          '    cert-file: /etc/pki/cert.pem',
          '    key-file: /etc/pki/private/key.key',
          '}',
          'listener {',
          '    addr: 0.0.0.0',
          '    port: 9672',
    ...
    context 'on hub client with ssl' do
      let :pre_condition do
        'class {"qpid::router":
           connector       => true,
           connector_name  => "hub",
